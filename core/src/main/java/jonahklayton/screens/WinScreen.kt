...
class WinScreen(private val passedLevel: Int) : KtxScreen {
    ...
    override fun render(delta: Float) {
        ...
        TextRenderer.begin(PlantGame.batch, viewport, TextRenderer.Font.LIGHT, 32f, .05f + kotlin.math.sin(time * 2 * PI * 4)
            .toFloat() * .05f)
        TextRenderer.drawTextCentered(0f, 100f, "You Beat Level $passedLevel!")
        menu.run(delta)
